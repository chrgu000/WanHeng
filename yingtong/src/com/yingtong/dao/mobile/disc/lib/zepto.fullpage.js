    ...
    $.extend(Fullpage.prototype, {
        ...
        start: function() {
            ...
            $this.on('touchstart', function(e) {
				...
				var targets = e.targetTouches;
				if( targets && targets.length > 0 )
					that.o.touch = targets[0];
				
                e.preventDefault();
				if( lockMoving ) return 0;
                if (that.movingFlag) {
                    return 0;
                }
            ...
            $this.on('touchend', function(e) {
				...
				if ( (sub > 30 || sub < -30) ) that.o.touch = null;
				var der = (sub > 30 || sub < -30) ? sub > 0 ? -1 : 1 : 0;
				
				if(that.o.touch)
				{
					var target = that.o.touch.target;
					if( ('length' in $(target) ) ) target = $($(target)[0]);
					if ( target.attr('onclick') ) eval(target.attr('onclick'));
				}
