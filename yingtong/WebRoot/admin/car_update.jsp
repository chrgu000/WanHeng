...
<body>
	<form action="${pageContext.request.contextPath}/car/updateCar.do"
		...
		<div
			...
			<div class="i_do_div rel" id="i_no_sku_stock_wrap">
					...
					<input type="text" id="i_no_sku_stock" name="name" value="${car.name }"
			...
			<div class="i_do_div rel" id="i_no_sku_stock_wrap">
				...
				<select name="vehicle_id" class="block input left tline">
					<c:forEach items="${vehicles }" var="v">
						<option value=${v. id }
							<c:if test="${car.vehicle_id==v.id }">selected</c:if>>${v.vehicle_model
							}</option>
					</c:forEach>
				</select>
			</div>
			<div class="i_do_div rel" id="i_no_sku_stock_wrap">
				...
				<select name="brand_id" class="block input left tline">
					<c:forEach items="${brands }" var="b">
						<option value=${b. id}
							<c:if test="${car.brand_id==b.id }">selected</c:if>>${b.name
							}</option>
					</c:forEach>
				</select>
			</div>
			<input type="hidden" id="input11" name="path" value="${car.path }" />
			<div id="pic11" class="i_do_div rel" id="i_no_sku_stock_wrap" >
				 <img 	src="../${car.path }" width="110" height="110" id="img11" />
			</div>
			<div class="i_do_div rel" id="i_no_sku_stock_wrap">
				...
				<div class="i_do_div rel" id="i_no_sku_stock_wrap">
					...
					<input type="text" id="i_no_sku_stock" name="original_price" value="${car.original_price }"
				...
				<div class="i_do_div rel" id="i_no_sku_stock_wrap">
					...
					<select name="price_id" class="block input left tline">
						<c:forEach items="${prices }" var="p">
							<option value="${p.id}"<c:if test="${car.price_id==p.id }">selected</c:if>>${p.price}</option>
						</c:forEach>
					</select>
				</div>
				<div class="i_do_div rel" id="i_no_sku_stock_wrap">
					<p class="i_do_tle r_txt abs font14">优惠价格</p>
					<input type="text" id="i_no_sku_stock" name="favourable_price" value="${car.favourable_price }"
			...
			<div class="i_do_div rel" id="i_no_sku_stock_wrap">
				...
				<select name="buyAddr" class="block input left tline">
					<c:forEach items="${addresses }" var="a">
						<option value="${a.addName }"
							<c:if test="${car. buyAddr==a.addName}">selected</c:if>>${a.addName}</option>
					</c:forEach>
				</select>
			</div>
			<div class="i_do_div rel" id="i_no_sku_stock_wrap">
				...
				<select name="sendAddr" class="block input left tline">
					<c:forEach items="${addresses }" var="a">
						<option value="${a.addName }"
							<c:if test="${car.sendAddr==a.addName }">selected</c:if>>
							${a.addName}</option>
					</c:forEach>
				</select>
			</div>
