...
(function(WIN, DOC) {
  ...
  setTranslate = function(ele, x, y, z, scale) {
    var name, prefix, _i, _len, _results;
    _results = [];
    for (_i = 0, _len = CSS_PREFIX_MAP.length; _i < _len; _i++) {
      prefix = CSS_PREFIX_MAP[_i];
      name = prefix ? "" + prefix + "Transform" : "transform";
      _results.push(ele.style[name] = "translate3d(" + (x || 0) + "px, " + (y || 0) + "px, " + (z || 0) + "px)"); // scale("+scale+","+scale+")
      //_results.push(ele.style[name] = "transform(" + (x || 0) + "px, " + (y || 0) + "px, " + (z || 0) + "px)");
    }
    return _results;
  };
  ...
  Slip = (function() {
    ...
    Slip.prototype.onTouchMove = function(event) {
	  ...
	  var canMoveY = absFinger.y > MIN_ALLOW_DISTANCE
      if (canMoveY) {
	  ...
	  this.coord.scale = 1 - Math.abs(this.coord.y / window.innerHeight);
	  
      setTranslate(ele, eleMove[X], eleMove[Y], 0, eleMove['scale']);
      _results = [];
      for (attr in eleMove) {
