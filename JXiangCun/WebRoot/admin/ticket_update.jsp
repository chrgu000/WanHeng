...
<body>
	<form action="../ticket/updateTicket.do" name="regForm"
		...
		<input type="hidden" name="id" value="${ticket.id }">
			...
			<div id="i_do_wrap">
				<div class="i_do_div rel" id="i_no_sku_stock_wrap">
					...
					<input type="text" id="price" name="price" onblur="checkPrice()" value="${ticket.price }"
				...
				<div class="i_do_div rel" id="i_no_sku_stock_wrap">
					...
					<c:if test="${ticket_title_id==t.id }">selected</c:if>
						<option value="${t.id }" 	<c:if test="${ticket.title_id==t.id }">selected</c:if>>${t.name}</option>
						</c:forEach>
					</select>
				</div>
				<div class="i_do_div rel" id="i_no_sku_stock_wrap">
					...
					<input type="text" id="endDate" name="endDate" 
						...
						placeholder="截止日期" value="<fmt:formatDate value="${ticket.endDate }" pattern="yyyy-MM-dd "/>"/>&nbsp;&nbsp;&nbsp;<label class="labelError"
						id="endDateError"></label>
				</div>
					<div class="i_do_div rel" id="i_no_sku_stock_wrap">
					<p class="i_do_tle r_txt abs font14">内容:</p>
					<input type="text" id="content" name="content" onblur="checkContent()"
						onfocus="removeError('content')" value="${ticket.content }" class="block input left tline"
