	...
	$.ajax( {
		...
		success : function(data) {
			...
			for ( var i = 0; i < data.length; i++) {
				var obj = data[i];
				$("#mark").append(
						"<input type='checkbox' name='markIds' value='"
								+ obj.id + "'>" + obj.name);
			}
		},
		error : function() {
...
function getArea() {
	...
	<body>
		<form
			action="${pageContext.request.contextPath}/merchant/updateMerchant.do"
			...
			<div
				...
				<div id="i_do_wrap">
					<div class="i_do_div rel" id="i_no_sku_stock_wrap">
						...
						<select name="city_id" class="block input left tline"
							...
							<c:forEach items="${citys }" var="c">
								<option value=${c. id}
									<c:if test="${merchant.city.id==c.id }">selected</c:if>>
									${c.name}
								</option>
							</c:forEach>
						</select>
					...
					<div class="i_do_div rel" id="i_no_sku_stock_wrap">
						...
						<select name="area_id" class="block input left tline"
							...
							<c:forEach items="${areas }" var="a">
								<option value=${a. id}
									<c:if test="${merchant.area.id==a.id }">selected</c:if>>
									${a.name}
								</option>
							</c:forEach>
						</select>
					...
					<div class="i_do_div rel" id="i_no_sku_stock_wrap">
						...
						<select name="sight_spot_id" class="block input left tline"
							...
							<c:forEach items="${sightSpots }" var="s">
								<option value=${s. id}
									<c:if test="${merchant.sightspot.id==s.id }">selected</c:if>>
									${s.name }
								</option>
							</c:forEach>
						</select>
					...
					<div class="i_do_div rel" id="i_no_sku_stock_wrap">
						...
						<select name="price_id" class="block input left tline">
							...
							<c:forEach items="${prices }" var="p">
								<option value="${p.id}"
									<c:if test="${merchant.price_id==p.id }">selected</c:if>>
									${p.price}
								</option>
							</c:forEach>
						</select>
					...
					<div class="i_do_div rel" id="i_no_sku_stock_wrap">
						...
						<input type="text" id="i_no_sku_stock" name="num"
							class="block input left tline" value="${merchant.num}"
							placeholder="序号" />
					</div>
					<div class="i_do_div rel" id="i_no_sku_stock_wrap">
						<p class="i_do_tle r_txt abs font14">
					...
					<div class="i_do_div rel" id="i_no_sku_stock_wrap">
						...
						<div id="title">
							<c:forEach items="${merchant.titles }" var="t">
								<input type="checkbox" name="titleIds" value="${t.id }"
									onclick="getMarks()"
									<c:forEach items="${merchant.titleIds}" var="m"><c:if test="${m==t.id}">checked</c:if> </c:forEach>>${t.name }
					</c:forEach>
						</div>
					</div>
					<div class="i_do_div rel" id="i_no_sku_stock_wrap">
						...
						<div id="mark">
							<c:forEach items="${marks }" var="m">
								<input type="checkbox" name="markIds" value="${m.id }"
									<c:forEach items="${merchant.markIds }" var="id" >
				<c:if test="${id==m.id }">checked</c:if>
				</c:forEach>>${m.name }
					</c:forEach>
						</div>
					</div>
					<div class="i_do_div rel" id="i_no_sku_stock_wrap">
						...
						<textArea id="content1" name="predetermine"
							style="width: 800px; height: 600px;">${merchant.predetermine }</textArea>
					</div>
