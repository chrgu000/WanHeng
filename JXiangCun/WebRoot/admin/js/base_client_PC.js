...
var M = {
    ...
    /*
    baseUrl : "",
    baseH5Url	: "http://wd.koudai.com",///H5_svn/WeiDian_H5/trunk/H5
    clientPort : 'http://login.koudai.com',//"<%= clientPort %>",
    clientTrs : 'http://wd.koudai.com',//"<%= clientTrs %>", //"http://v.vdian.com",//http://logintest2.koudai.com
    clientOrd : "http://wd.koudai.com",
*/

    baseUrl:"/vshop/1/CPC",
    baseH5Url:"http://wd.koudai.com",
    clientPort:"http://login.koudai.com",
    clientTrs:"http://v.vdian.com",
    clientOrd:"http://wd.koudai.com",
    version:628,
    doc : document,
    win : window,
    ...
    setCookie : function(name,value,isSession){
        ...
        var path_domain = "; path=/"+ (document.domain.indexOf("vdian.com") != -1 ? "; domain=vdian.com" : (document.domain.indexOf("koudai.com") != -1 ? "; domain=koudai.com" : "") );
    ...
    clearCookie : function(){
        ...
        if (_keys){
            var l = _keys.length;
            for(var i = l; i--;){ M.delCookie(_keys[i]); }
        }
    },
	ua : function(){
	...
	/*alert & confirm start 需要改造 length() to length*/
	...
	_prompt : function(tle,val,btnA,btnB,submit_fun,cancel_fun){
		var _d = document;
		var _prompt_bg = _d.createElement("div");
		_prompt_bg.setAttribute("id","_prompt_bg");
		_d.body.appendChild(_prompt_bg);
				
		var _prompt_content = _d.createElement("div");
		_prompt_content.setAttribute("id","_prompt_content");
		_prompt_bg.appendChild(_prompt_content);
		
		var _wrap = $("#_prompt_content");
		
		var _temp = "";
		_temp = _temp + "<p>"+ tle +"</p>";
       _temp = _temp + "<div id='_prompt_textareaW'>";
           _temp = _temp + "<textarea id='_prompt_textarea'>"+ val +"</textarea>";
       _temp = _temp + "</div>";        
		_temp = _temp + "<em id='_prompt_shadowA'>&nbsp;</em>";
		_temp = _temp + "<em id='_prompt_shadowB'>&nbsp;</em>";
		_temp = _temp + "<div id='_prompt_btnW'>";
			if(btnB[0]){//B按钮有
				_temp = _temp + "<div id='_prompt_btnA' class='"+ btnA[1] +"'>"+ btnA[0] +"</div>";
				_temp = _temp + "<em id='_prompt_shadowC'>&nbsp;</em>";
				_temp = _temp + "<em id='_prompt_shadowD'>&nbsp;</em>";
				_temp = _temp + "<div id='_prompt_btnB' class='"+ btnB[1] +"'>"+ btnB[0] +"</div>";
			}
			else{
				_temp = _temp + "<div id='_prompt_btnA' class='"+ btnA[1] +"' style='width:100%'>"+ btnA[0] +"</div>";
			}
		_temp = _temp + "</div>";
	
		_wrap.html(_temp).fadeIn();
		
		$("#_prompt_btnA").bind("click",function(){/*cancel*/
			cancel_fun && cancel_fun();
			_wrap.fadeOut(function(){
				$("#_prompt_bg").remove();
			})
		...
		if(btnB[0]){//B按钮有
			$("#_prompt_btnB").bind("click",function(){/*submit*/
                if(!$("#_prompt_textarea").val()){
                    M._alert("请填写内容");
                    return;
                }
            
				submit_fun($("#_prompt_textarea").val());
				_wrap.fadeOut(function(){
					$("#_prompt_bg").remove();
				})
			})
	...
	init : function(){
        if(M.ua().indexOf("msie 6.") != -1 || M.ua().indexOf("msie 7.") != -1 || M.ua().indexOf("msie 8.") != -1 || M.ua().indexOf("msie 9.") != -1){console = new Object(); console.log = function(s){ return ''; };}

        var _domain = document.domain;
        if(_domain.indexOf("vdian.com") != -1){
            //document.domain = "vdian.com";
            if(Number(M.getCookie("WD_clear_domain")) !== 1){
                console.log("begin_clear_domain");
                var _delCookie = function(name){
                    document.cookie = escape(name) +"=; expires="+ new Date(0).toUTCString() + "; path=/";
                    document.cookie = escape(name) +"=; expires="+ new Date(0).toUTCString() + "; path=/; domain=v.vdian.com";
                }
                var _clearCookie = function(){
                    var _keys = document.cookie.match(/[^ =;]+(?=\=)/g);
                    if (_keys){
                        var l = _keys.length;
                        for(var i = l; i--;){ _delCookie(_keys[i]); }
                    }
                }
                _clearCookie();
        ...
        if($("#hd_u_name").length){
            if(M.getCookie("WD_client_user_true_name") && Number(M.getCookie("WD_client_wduss_raw").indexOf("%")) === -1){//有真实姓名 并且 没有百分号
                $("#hd_u_name").html(M.getCookie("WD_client_user_true_name") +"，你好，欢迎使用微店网页版").show();
            }
            else{
                location.href = M.baseUrl +"/login.php";
            }
        }
        //add 08-15 by yinliusheng  ps 供货商判断
        if(Number(M.getCookie('seller_identity'))===2){
        	$('#retailDisplay').removeClass('hide');
...
if(window != window.top){
	...
	if(localurl.indexOf('item_do_img_direct.php')<0){
...
(function(){
    var is_https = ("https:" === document.location.protocol) ? 1 : 0,
        u = (is_https ? "https" : "http") + "://analysis.koudai.com/";
    ...
    g.defer=true;
    g.async=true;
    g.src=is_https ? 'https://analysis.koudai.com/piwik.js' : 'http://static.koudai.com/analysis/piwik.js';
...
ga('create', 'UA-46806553-2', 'vdian.com');
ga('send', 'pageview');