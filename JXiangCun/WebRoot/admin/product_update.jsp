...
<html>
	...
	<head>
		...
		<script type="text/javascript" src="../admin/js/rili.js">
</script>
		<script type="text/javascript">
KE
		.show( {
	...
	$("input[type='radio']").each(function() {
		...
		if ($("#" + id).attr("checked") == "checked") {
			...
			if (value == 0) {
				...
				document.regForm.original_price.value = 0;
				document.regForm.favourable_price.value = 0;
...
function getTtiles() {
	...
	<body>
		<form
			action="${pageContext.request.contextPath}/product/updateProduct.do"
				...
				<div id="i_do_wrap">
					<div class="i_do_div rel" id="i_no_sku_stock_wrap">
						...
						<select name="merchant_id" class="block input left tline"
							onchange="getTtiles()" id="merchant_id">
							<option value="">
								--选择商户--
							</option>
							<c:forEach items="${merchants}" var="m">
								<option value="${m.id}"
									<c:if test="${product.merchant.id==m.id }">selected</c:if>>
									${m.title}
								</option>
							</c:forEach>
						</select>
					...
					<div class="i_do_div rel" id="i_no_sku_stock_wrap">
						...
						<input type="radio" name="isFree" value="1" id="isFree1"
							onclick="checkFree(this.value)"
							<c:if test="${product.isFree=='1' }">checked</c:if>>
						是
						<input type="radio" name="isFree" value="0" id="isFree2"
							onclick="checkFree(this.value)"
							<c:if test="${product.isFree=='0' }">checked</c:if>>
						否
					</div>
					<div id="i_no_sku_stock_wrap9">
						<div class="i_do_div rel" id="i_no_sku_stock_wrap">
							...
							<input type="text" name="pointDate" id="pointDate"
								...
								value="<fmt:formatDate value="${product.pointDate }" pattern="yyyy-MM-dd "/>" />
							&nbsp;&nbsp;&nbsp;
						</div>
					</div>
					<div id="i_no_sku_stock_wrap1">
						<div class="i_do_div rel" id="i_no_sku_stock_wrap">
							...
							<input type="text" id="original_price" name="original_price"
								...
								value="${product.datePrices[0].original_price }" />
						</div>
					</div>
					<div id="i_no_sku_stock_wrap2">
						<div class="i_do_div rel" id="i_no_sku_stock_wrap">
							...
							<input type="text" id="favourable_price" name="favourable_price"
								...
								value="${product.datePrices[0].favourable_price }" />
						</div>
					</div>
					<div id="i_no_sku_stock_wrap5">
						<div class="i_do_div rel" id="i_no_sku_stock_wrap">
							...
							<input type="text" id="day1_original_price"
								...
								placeholder="周一~~周四原价"
								value="${product.datePrices[1].original_price }" />
						</div>
					</div>
					<div id="i_no_sku_stock_wrap6">
						<div class="i_do_div rel" id="i_no_sku_stock_wrap">
							...
							<input type="text" id="day1_favourable_price"
								...
								placeholder="周一~~周四现价"
								value="${product.datePrices[1].favourable_price }" />
						</div>
					</div>
					<div id="i_no_sku_stock_wrap7">
						<div class="i_do_div rel" id="i_no_sku_stock_wrap">
							...
							<input type="text" id="day5_original_price"
								...
								placeholder="周五~~周日原价"
								value="${product.datePrices[2].original_price }" />
						</div>
					</div>
					<div id="i_no_sku_stock_wrap8">
						<div class="i_do_div rel" id="i_no_sku_stock_wrap">
							...
							<input type="text" id="day5_favourable_price"
								...
								placeholder="周五~~周日现价"
								value="${product.datePrices[2].favourable_price }" />
						</div>
					</div>
					<div id="i_no_sku_stock_wrap3">
						<div class="i_do_div rel" id="i_no_sku_stock_wrap">
							<p class="i_do_tle r_txt abs font14">
								免费截止日期:
							</p>
							<input type="text" id="endDate" name="endDate"
								class="block input left tline" placeholder="免费截止日期"
								value="<fmt:formatDate value="${product.endDate }" pattern="yyyy-MM-dd "/>" />
						</div>
					</div>
					<div id="i_no_sku_stock_wrap4">
						<div class="i_do_div rel" id="i_no_sku_stock_wrap">
							<p class="i_do_tle r_txt abs font14">
								免单数量:
							</p>
							<input type="text" id="endDate" name="free_num"
								class="block input left tline" placeholder="免单数量"
								value="${product.free_num }" />
						</div>
					</div>
					<input type="text" id="input11" style="display: none;" name="path"
						value="${product.path }" />
					<div id="pic11" class="i_do_div rel" id="i_no_sku_stock_wrap">
						<img src="../${product.path }" width="110" height="110" id="img11" />
					</div>
					<div class="i_do_div rel" id="i_no_sku_stock_wrap">
						<p class="i_do_tle r_txt abs font14">
							图片展示
						...
						<div id="title">
							<c:forEach items="${titles }" var="t">
								<input type="checkbox" name="titleIds" value="${t.id }"
									<c:forEach items="${product.titleIds}" var="m"><c:if test="${m==t.id}">checked</c:if> </c:forEach>>${t.name }
					</c:forEach>
						</div>
					</div>
