...
if (document.regForm.area_id.value == "") {
	...
	<body>
		<form
			action="${pageContext.request.contextPath}/sightSpot/updateSightSpot.do"
			name="regForm" onsubmit="return check()" method="post">
			<input type="hidden" name="id" value="${sightSpot.id }">
			<div
				...
				<div class="i_do_div rel" id="i_no_sku_stock_wrap">
					...
					<input type="text" id="i_no_sku_stock" name="name"
						value="${sightSpot.name }" class="block input left tline"
						placeholder="景点名称" />
				</div>
				<div class="i_do_div rel" id="i_no_sku_stock_wrap">
					<p class="i_do_tle r_txt abs font14">
					...
					<select name="city_id" class="block input left tline" id="city"
						onchange="getArea()">
						<option value="">
							--选择城市--
						</option>
						<c:forEach items="${citys }" var="c">
							<option value=${c. id}
								<c:if test="${sightSpot.city_id==c.id }">selected</c:if>>
								${c.name}
							</option>
						</c:forEach>
					</select>
				...
				<div class="i_do_div rel" id="i_no_sku_stock_wrap">
					...
					<select name="area_id" class="block input left tline" id="area">
						<option value="">
							--区/县--
						</option>
						<c:forEach items="${areas}" var="a">
							<option value="${a.id}"
								<c:if test="${sightSpot.area_id==a.id}">selected</c:if>>
								${a.name}
							</option>
						</c:forEach>
					</select>
				</div>
				<input type="hidden" id="input11" name="path"
					value="${sightSpot.path }" />
				<div id="pic11" class="i_do_div rel" id="i_no_sku_stock_wrap">
					<img src="../${sightSpot.path }" width="110" height="110"
						id="img11" />
				</div>
				<div class="i_do_div rel" id="i_no_sku_stock_wrap">
					<p class="i_do_tle r_txt abs font14">
				...
				<div class="i_do_div rel" id="i_no_sku_stock_wrap">
					...
					<input type="text" id="i_no_sku_stock" name="content"
						value="${sightSpot.content}" class="block input left tline"
						placeholder=" 内容" />
				</div>
				<div class="i_do_div rel" id="i_no_sku_stock_wrap">
					<p class="i_do_tle r_txt abs font14">
				...
				<div class="i_do_div rel" id="i_no_sku_stock_wrap">
					...
					<c:forEach items="${titles }" var="t">
						<input type="checkbox" name="titleIds" value="${t.id }"
							<c:forEach items="${sightSpot.titles}" var="s">
							<c:if test="${s.id==t.id}">checked</c:if>
							</c:forEach>>${t.name }
					</c:forEach>
				</div>
