...
<body>
	<form action="${pageContext.request.contextPath}/search/searchList.do"
		...
		<div
			...
			<div id="">
				<div class="pro_sou">
					<a href="${pageContext.request.contextPath}/search/toAddSearch.do"
						...
						style="margin-left:20px;margin-right:20px ;">搜索标题:<input
						type="text" name="title" value="${sessionScope.searchPage.title }"
					...
					<div id="list_wrap" class="rel " style="width:98%;margin-top: 17px">
						<table style="width:100%">
							<tr
								...
								<td style="width:16%;" class="xiahuaxian">搜索标题</td>
								<td style="width:20%;" class="xiahuaxian">内容</td>
								<td style="width:16%;" class="xiahuaxian">链接</td>
								<td style="width:24%;" class="xiahuaxian">序号</td>
							...
							<c:forEach items="${searchs}" var="s">
								<tr
									...
									<td class="xiahuaxian">${s.title}</td>
									<td class="xiahuaxian">${s.chr}</td>
											<td class="xiahuaxian">${s.url}</td>
									<td class="xiahuaxian">${s.num }</td>
									<td class="xiahuaxian"><a
										href="${pageContext.request.contextPath}/search/toUpdateSearch.do?id=${s.id}">修改信息</a>
										&nbsp;|&nbsp;<a
										href="${pageContext.request.contextPath}/search/deleteSearch.do?id=${s.id}"
										onclick="return confirm('删除之后数据将不可恢复，确认删除吗？');">删除</a></td>
								</tr>
							</c:forEach>
									<tr style="text-align:center; line-height:60px; ">
								<td colspan="5">
									第${searchPage.currentPage}页/共${searchPage.totalPage}页&nbsp;&nbsp;
									<a class="inline_b btns_2"
									href="${pageContext.request.contextPath}/search/searchList.do?currentPage=1">首页</a>&nbsp;&nbsp;
									<a class="inline_b btns_2"
									href="${pageContext.request.contextPath}/search/searchList.do?currentPage=${searchPage.currentPage-1==0?1:searchPage.currentPage-1}">上一页</a>&nbsp;&nbsp;
									<c:forEach begin="1" end="${searchPage.totalPage}" var="num">
										<a class="inline_b btns_2" style="width:30px;"
											href="${pageContext.request.contextPath}/search/searchList.do?currentPage=${num}">${num
											}</a>
									</c:forEach> &nbsp;&nbsp; <a class="inline_b btns_2"
									href="${pageContext.request.contextPath}/search/searchList.do?currentPage=${searchPage.currentPage+1>searchPage.totalPage?searchPage.totalPage:searchPage.currentPage+1}">下一页</a>&nbsp;&nbsp;
									<a class="inline_b btns_2"
									href="${pageContext.request.contextPath}/search/searchList.do?currentPage=${searchPage.totalPage}">尾页</a>
									<a class="inline_b btns_2"
									...
									style="height:35px;width:60px;float: right;">
										<c:forEach begin="1" end="${searchPage.totalPage}" var="num">
											<option value="${num}" 	<c:if test="${searchPage.currentPage==num }">selected</c:if> style="font-szie:14px;color:#434343;">第${num}页</option>
										</c:forEach>
								</select> <script type="text/javascript">
									function jump() {
										var num = document.getElementById("s1").value;
										window.location.href = "${pageContext.request.contextPath}/search/searchList.do?currentPage="
												+ num;
									}
								</script></td>
							</tr>
