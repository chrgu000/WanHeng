...
(function( $ ) {
	...
	function createBox( $fileInput, file, webUploader ) {
		...
		//添加子容器;
		var li = '<li id="fileBox_'+file_id+'" class="diy" class="diyUploadHover"> \
		...
		var $diyCancel;
	 	var $files = $parentFileBox.children('.fileBoxUl').find(".diy");
		$files.each(function(){
			//绑定取消事件;
			$diyCancel=$(this).children('.diyCancel').one('click',function(){
		...
		webUploader.makeThumb( file, function( error, dataSrc ) {
			if ( !error ) {	
				$fileBox.find('.viewThumb').append('<img src="'+dataSrc+'" class="image" class="img">');
			}
		});	
	}
	//获取文件类型;
...
function put(){
	var $files = $('.fileBoxUl').find("li");
	$files.each(function(){
		//绑定取消事件;
		$(this).children('.diyCancel').one('click',function(){
			var id=$($(this).siblings(".viewThumb")[0]).children(".img").attr("id");
			if(id){
				$.ajax({
					url: "../../picture/deletePictureById.do",
					type: "post",
					dataType: "json",
					data: {id:id},
