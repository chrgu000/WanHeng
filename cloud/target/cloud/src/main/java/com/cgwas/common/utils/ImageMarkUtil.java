...
public class ImageMarkUtil {  
    ...
    /** 水印图片旋转角度 */  
    private static double degree = 0d;  
     ...
     *            水印图片间隔 
     */  
    public static void setImageMarkOptions(float alpha, double degree,  
            int interval) {  
        if (alpha != 0.0f) {  
     ...
     *            目标图片路径 
     */  
    public static void waterMarkByImg1(String waterImgPath, String srcImgPath,String targerPath) {  
        try {  
            waterMarkByImg(waterImgPath, srcImgPath, targerPath, 0);  
        } catch (Exception e) {  
        ...
        try {  
            ...
            for (int height = interval + imgIcon.getIconHeight(); height < buffImg  
                ...
                for (int weight = interval + imgIcon.getIconWidth(); weight < buffImg  
                    ...
                    if(weight > buffImg.getWidth()){
                    	break;
                    }
                }  
                if(height > buffImg.getHeight()){
                	break;
                }
            }  
    ...
    public static void main(String[] args) {  
         ...
         */  
        //修改默认参数  
        ImageMarkUtil.setImageMarkOptions(0.5f, degree,0);  
        String watermarkPath = "E:/temp/watermark.png";  //测试水印图片  
        String imgPath = "E:/temp/c3.jpg";//测试需加水印图片  
        String targetPath = "E:/temp/c2.jpg";//生成的图片  
        ImageMarkUtil.waterMarkByImg1(watermarkPath, imgPath,targetPath);  
        long end = System.currentTimeMillis();
        System.out.println("..添加水印图片结束..."+(end - start));  
    }  
