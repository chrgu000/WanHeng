...
import com.fengyun.video.util.SearchTransCodeInfo;
...
public class CourseAction {
	...
	public Result getMySelfCourse(String type, HttpSession session,
			Integer pageSize, Integer pageNo, Course course) {
		JSONObject user = JSONObject.parseObject(session.getAttribute(
				"loginUser").toString());
	...
	public Result payCourse(HttpSession session, String password,
			Long course_id, HttpServletRequest request) {
		JSONObject user = JSONObject.parseObject(session.getAttribute(
				"loginUser").toString());
		Long userId = user.getLong("id");
		...
		JSONObject userPrice = JSONObject.parseObject(result).getJSONObject(
				"data");
		...
		// 查询余额是否充足
		if (retn.getInteger("remaining_sum") == null
				|| (retn.getInteger("remaining_sum") - u <= 0)) {// 判断余额是否充足
		...
		HttpUtil.doPost(HTTPConfig.HTTP_PREFIX
				+ "/cgwas/cloud/accessUserMoney.action", walletInfoOut);
		// 转账--讲师收款
		Map<String, Object> walletInfo2 = new HashMap<String, Object>();
		walletInfo2.put("user_id", c.getTeacher().getUser_id());
		walletInfo2.put("remaining_sum",
				CommonUtil.FormetFileSize(Double.valueOf(c.getPrice()), 0.95));
		walletInfo2.put("flag", "in");
		HttpUtil.doPost(HTTPConfig.HTTP_PREFIX
				+ "/cgwas/cloud/accessUserMoney.action", walletInfo2);
		// 转账--平台收钱
		Map<String, Object> walletInfoIn = new HashMap<String, Object>();
		walletInfoIn.put("user_id", 1L);
		walletInfoIn.put("remaining_sum",
				CommonUtil.FormetFileSize(Double.valueOf(c.getPrice()), 0.05));
		walletInfoIn.put("flag", "in");
		HttpUtil.doPost(HTTPConfig.HTTP_PREFIX
				+ "/cgwas/cloud/accessUserMoney.action", walletInfoIn);
		...
		buffer.append("【" + userName + "学员】购买了您的【" + c.getCourse_name()
				+ "】系列课程,对方转账给您【"
				+ CommonUtil.FormetFileSize(Double.valueOf(c.getPrice()), 0.95)
				+ "】元。");
		...
		HttpUtil.doPost(HTTPConfig.HTTP_PREFIX
				+ "/cgwas/cloud/addTradeRecord.action", tradeRecord);

		Map<String, Object> tradeRecordOut = new HashMap<String, Object>();
		StringBuffer bufferOut = new StringBuffer();
		bufferOut.append("您购买了【" + c.getTeacher().getName() + "讲师】的【"
				+ c.getCourse_name() + "】系列课程，支付了【" + c.getPrice() + "】元");
		tradeRecordOut.put("trade_content", bufferOut.toString());
		tradeRecordOut.put("trade_order_no",
				String.valueOf(new Date().getTime()));
		tradeRecordOut.put("trade_type", "7");
		tradeRecordOut.put("trade_price",
				CommonUtil.FormetFileSize(Double.valueOf(c.getPrice()), 1.00));
		...
		HttpUtil.doPost(HTTPConfig.HTTP_PREFIX
				+ "/cgwas/cloud/addTradeRecord.action", tradeRecordOut);

		Map<String, Object> tradeRecordIn = new HashMap<String, Object>();
		StringBuffer bufferIn = new StringBuffer();
		bufferIn.append("【" + userName + "学员】购买了贵平台中【"
				+ c.getTeacher().getName() + "讲师】的【" + c.getCourse_name()
				+ "】系列课程,付手续费【"
				+ CommonUtil.FormetFileSize(Double.valueOf(c.getPrice()), 0.05)
				+ "】元。");
		tradeRecordIn.put("trade_content", bufferIn.toString());
		tradeRecordIn.put("trade_order_no",
				String.valueOf(new Date().getTime()));
		tradeRecordIn.put("trade_type", "6");
		tradeRecordIn.put("trade_price",
				CommonUtil.FormetFileSize(Double.valueOf(c.getPrice()), 0.05));
		...
		HttpUtil.doPost(HTTPConfig.HTTP_PREFIX
				+ "/cgwas/cloud/addTradeRecord.action", tradeRecordIn);

		Map<String, Object> map = new HashMap<String, Object>();
		map.put("user_id", userId);
		map.put("course_id", course_id);
		map.put("type", "1");
		LearnerCourse learner_course = learnerCourseService
				.getLearnerCourseByUserMap(map);
	...
	public Result getAllInterestDirections(Integer type_id) {
		List<InterestDirection> interestDirections = interestDirectionService
				.getAllInterestDirections(type_id);
	...
	public Result getSoftwaresByInterestDirectionId(
		...
		List<Software> softwares = softwareService
				.getSoftwaresByInterestDirectionId(interest_direction_id);
		return new Result(Result.SUCCESS, "成功", softwares);
	}
	...
	public Result getAllTradeSkills(Integer type_id) {
		List<TradeSkill> tadeSkills = tradeSkillService
	...
	public Result previewImgFile(MultipartFile userfile,
		...
		if (!userfile.isEmpty()) {
			try {
				...
				// 通过base64来转化图片
				base64Data = base64Data.append(encoder.encode(userfile
						.getBytes()));
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public Result getCoursesByUserMap1(Integer pageSize, Integer pageNo,
			Course course, HttpSession session) {
		Map<String, Object> map = new HashMap<String, Object>();
		Page page = null;
		try {
			Map<String, String> params = new HashMap<String, String>();
	...
	public Result getCoursesByAdmin(Integer pageSize, Integer pageNo,
		...
		try {
			...
			params.put("pageNo", String.valueOf(pageNo));
			page = PageUtils.createPage(params);
			page = courseService.page5(page, course);
			map.clear();
			map.put("pageMax",
					Double.valueOf(
							Math.ceil(page.getTotal() * 1.0 / page.getLimit()))
	...
	@ResponseBody
	public Result searchCourseList(Integer pageSize, Integer pageNo,
			Course course, Long trade_skill_id, Long software_id,
			Integer type_id, Integer program_id, String news, String mood) {
		Map<String, Object> map = new HashMap<String, Object>();
		Page page = null;
		try {
			Map<String, String> params = new HashMap<String, String>();
	...
	public @ResponseBody
	Result getChapterOrderOfCourse(Long course_id) {
		List<Chapter> chapters = courseService
				.getChapterOrderOfCourse(course_id);
	...
	public @ResponseBody
	Result getChaptersByCourseId(Long course_id) {
	...
	public Result uploadImg(@RequestBody Map<String,String> cover) {
		...
		String imgFileUrl = "cgwas/cloud/images/";
		if (course_cover != null && course_cover.length() > 50) {
			course_cover = course_cover
					.substring(course_cover.indexOf(",") + 1);
	...
	@Transactional
	public @ResponseBody
	Result createCourse(Course course, Chapter chapter, String tradeSkillIds,
			String softwareIds, HttpSession session, Integer type_id,
			Integer program_id, String program_content, String beginDate,
			String endDate, String onliveTime) throws Exception {
		if (course != null && chapter != null) {
			...
			if (chapter.getCourse_id() == null) {
				...
				if ("Y".equals(course.getIs_free())
						&& !"Y".equals(course.getIs_apply())) {
				...
				if (null == program_id && p.getContent() != null
			...
			if ("N".equals(course.getIs_free())) {
				...
				HttpUtil.doPost(HTTPConfig.HTTP_PREFIX
						+ "/cgwas/wxMessageSendApi/applySend.json", query);
			}
	...
	@Transactional
	public @ResponseBody
	Result update(Course course, Chapter chapter, Long cid, Long chapter_id,
			String tradeSkillIds, String softwareIds, Integer type_id,
			Integer program_id, String program_content, String beginDate,
			String endDate, String onliveTime) throws Exception {
		if (course != null && chapter != null) {
			...
			if (course.getIs_apply().equals("Y")) {
				chapter.setCheck_status("C");
				if (c != null && "Y".equals(c.getIs_public())
						&& "Y".equals(c.getIs_apply())) {
					course.setIs_public("Y");
				} else {
					course.setIs_public("N");
				}
	...
	public @ResponseBody
	Result joinCourse(Long course_id, HttpSession session) {
		JSONObject user = JSONObject.parseObject(session.getAttribute(
				"loginUser").toString());
		Long userId = user.getLong("id");
		Map<String, Object> map = new HashMap<String, Object>();
	...
	public Result getCollectCourseOfMyOrNot(Long course_id, HttpSession session) {
		...
		if (user != null) {
			...
			LearnerCourse learner_course = learnerCourseService
					.getLearnerCourseByUserMap(map);
			if (learner_course == null
					|| "Y".equals(learner_course.getDelflag())
					&& "3".equals(learner_course.getType())) {
	...
	public @ResponseBody
	Result collectCourse(Long course_id, HttpSession session) {
		JSONObject user = JSONObject.parseObject(session.getAttribute(
				"loginUser").toString());
		Long userId = user.getLong("id");
		Map<String, Object> map = new HashMap<String, Object>();
	...
	public Result getOnlineSite(String course_id, String chapter_id) {
		String teacher_key = CreateKey.getTeachKey(
				course_id + "_" + chapter_id, chapter_id);
		String student_key = CreateKey.getStudyKey(
				course_id + "_" + chapter_id, chapter_id);
		Chapter chapter = new Chapter();
		...
		if ("Y".equals(c.getIs_save_video())) {
			CreateVideoRecord
					.doExeUrl(course_id + "_" + chapter_id, chapter_id);
		}
	 ...
	 * @throws IOException
	 */
	@RequestMapping("/getMakeFileUrl")
	@ResponseBody
	public Result getMakeFileUrl(String type) {
		net.sf.json.JSONObject jsonObject = null;
		Map<String,Object> cache=new HashMap<String,Object>();
		try {
			StringBuffer fileUrls = null;
			if ("video".equals(type)) {
				fileUrls = new StringBuffer("cgwas/cloud/videos/");
			} else {
				fileUrls = new StringBuffer("cgwas/cloud/images/"+((Math.random()+"").substring(3))+"/");
			}
			if (!OSSFilesUtil.isFile(fileUrls.toString())) {
				OSSFilesUtil.addFile(fileUrls.toString());
			}
			jsonObject = OSSFilesUtil.postObjectPolicy(fileUrls.toString());
			cache.put("postObjectPolicy", jsonObject);
		} catch (Exception e) {
			e.printStackTrace();
			return new Result("SYS_ASK_ERRO", null);
		}
		return new Result(Boolean.TRUE, "成功!", cache);
	}
	 ...
	 * @throws IOException
	 */
	@RequestMapping("/uploadMakeFileSuccess")
	@ResponseBody
	public Result uploadMakeFileSuccess(String fileName,String chapter_id) throws Exception {
		String newFileName = "cgwas/cloud/videos/"+System.currentTimeMillis() + ""
				+ new Random().nextInt(1000) + ".mp4";
		Map<String,String> map=new HashMap<String,String>();
		map.put("chapter_id", chapter_id);
		map.put("video_url", newFileName);
