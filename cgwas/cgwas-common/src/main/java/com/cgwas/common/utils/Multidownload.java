...
public class Multidownload {
	...
	public void download() throws Exception {
		...
		if (code == 200) {
			...
			// 删除压缩包
			File oldpath = new File(url.getFile());
			cleanTemp(oldpath);
	...
	private class DownloadThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				// 加载下载位置的文件
				File downThreadFile = new File(targetFilePath, "downThread_"
						+ threadId + ".dt");
				RandomAccessFile downThreadStream = null;
				if (downThreadFile.exists()) {// 如果文件存在
					downThreadStream = new RandomAccessFile(downThreadFile,
							"rwd");
				...
				System.out.println("线程_" + threadId + "的下载起点是 " + startIndex
						+ "  下载终点是: " + endIndex);
				...
				if (connection.getResponseCode() == 206) {// 200：请求全部资源成功，
					...
					while ((length = inputStream.read(buffer)) > 0) {
						 ...
						 * 将当前现在到的位置保存到文件中
						 */
						System.out.println("下载中请等待。。。");
						downThreadStream.seek(0);
						downThreadStream.write((startIndex + total + "")
								.getBytes("UTF-8"));
					}
	...
	public static void main(String[] args) {
		try {
			new Multidownload("http://localhost:8080/cgwas-webapp/js/js.zip",
					null, 3).download();
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 * @param threadCount
	 */
	public static void download(String path, String targetFilePath,
			int threadCount) {
		try {
			new Multidownload(path, targetFilePath, threadCount).download();
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
