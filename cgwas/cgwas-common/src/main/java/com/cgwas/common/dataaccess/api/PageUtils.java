...
public class PageUtils {
	...
	public static Page createPage1(String prefix,Map<String,String> map) {
		FastPage page = new FastPage();
		Long rowCount = pasLong(map.get( "rowCount"));
		Long start = pasLong(map.get( "start"));
		Long total = pasLong(map.get( "total"));
		Long limit = pasLong(map.get( "pageSize"));
		
		Long currentPage = pasLong(map.get( "pageNo"));
		Long nextPage = pasLong(map.get( "nextPage"));
		Object orderByPropertyStart = pasObject(map.get( "orderByPropertyStart"));
		Object orderByPropertyEnd = pasObject(map.get( "orderByPropertyEnd"));
		if (limit == null || limit == 0) {
			limit = DEFAULT_LIMIT;
		}
		if (rowCount != null) {
	 ...
	 * @return 分页对象
	 */
	public static Page createPage(Map<String,String> map) {
		return createPage1(null, map);
	}
